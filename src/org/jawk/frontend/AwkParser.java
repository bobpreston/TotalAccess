...
public class AwkParser {
	...
	static {
		...
		// RMP
        SPECIAL_VAR_NAMES.put("IT", sp_idx);
        SPECIAL_VAR_NAMES.put("IFS", sp_idx);
        SPECIAL_VAR_NAMES.put("DATEFMT", sp_idx);
        SPECIAL_VAR_NAMES.put("SYSPROPS", sp_idx);
        SPECIAL_VAR_NAMES.put("_$NODE", sp_idx);
        SPECIAL_VAR_NAMES.put("DEFPROPS", sp_idx);
        SPECIAL_VAR_NAMES.put("NS", sp_idx);
	}
			...
			throws IOException
	{
		...
		// RMP
//		System.out.println("*** Its " + assertOneLexerThread() + " " + Thread.currentThread().getId());
//		try {
//            Thread.sleep(1000);
...
////				read();
		...
		if (c == '.') {
			...
			if (!hit) {
			    return token = this._BUILTIN_FUNC_NAME_;
		...
		if (Character.isDigit(c)) {
			...
			while (c > 0) {
				if (c == '.') {
					// double!
					read();
					while (c > 0 && (Character.isDigit(c) || ("eE+-".indexOf(c)) != -1)) {
						read();
					}
					return token = _DOUBLE_;
				// RMP
				} else if (Character.isDigit(c) || ("xXabcdefABCDEF".indexOf(c) != -1)) {
					// integer or double.
					read();
				} else {
					break;
			...
			while (token != _EOF_ && c != '"' && c != '\n') {
				...
				if (c == '\\') {
					...
					if (c == 't') {
                        ...
                        string.append('\u000b');
                    } else if ("01234567".indexOf(c) != -1) {
                        // Octal
                        StringBuilder value = new StringBuilder();
                        value.append("\\");
                        reader.mark(2);
                        int c1 = c;
                        value.append((char)c1);
                        read();
                        int c2 = c;
                        if ("01234567".indexOf(c2) != -1) {
                            value.append((char)c2);
                            reader.mark(1);
                            read();
                            int c3 = c;
                            if ("01234567".indexOf(c3) != -1) {
                                value.append((char)c3);
                                reader.mark(0);
                            }
                        }
                        reader.reset();
                        string.append(StringEscapeUtils.unescapeJava(value.toString()));
                    } else if (c == 'u') {
                        // Unicode
                        StringBuilder value = new StringBuilder();
                        value.append("\\u");
                        ...
                        read();
                        value.append((char)c);
                        c = Integer.parseInt(value.toString(), 16);
			...
			throws IOException
	{
		...
		if (token == _OPEN_BRACKET_) {
			...
			// RMP
			while (token == _OPEN_BRACKET_) {
//				throw new ParserException("Use [a,b,c,...] instead of [a][b][c]... for multi-dimensional arrays.");
	            lexer();
			    AST idx2 = ARRAY_INDEX(allow_comparators, allow_in_keyword);
	            lexer(_CLOSE_BRACKET_);
			    idx_ast = new ArrayIndex_AST2(idx2, idx_ast);
			}
	...
	*/
	...
	private final class RuleList_AST extends AST {
		...
		public int populateTuples(AwkTuples tuples) {
			...
			// RMP
            ID_AST it_ast = symbol_table.getID("IT");
            ID_AST ifs_ast = symbol_table.getID("IFS");
            ID_AST datefmt_ast = symbol_table.getID("DATEFMT");
            ID_AST sysprops_ast = symbol_table.getID("SYSPROPS");
            ID_AST node_ast = symbol_table.getID("_$NODE");
            ID_AST defprops_ast = symbol_table.getID("DEFPROPS");
            ID_AST ns_ast = symbol_table.getID("NS");
			...
			boolean b
                    ...
                    = it_ast.is_scalar
                    = ifs_ast.is_scalar
                    = datefmt_ast.is_scalar
                    = sysprops_ast.is_array // note!
                    = defprops_ast.is_array // note!
                    = ns_ast.is_array // note!
                    = node_ast.is_scalar
			...
			// RMP
            tuples.itOffset(it_ast.offset);
            tuples.ifsOffset(ifs_ast.offset);
            tuples.datefmtOffset(datefmt_ast.offset);
            tuples.syspropsOffset(sysprops_ast.offset);
            tuples.nodeOffset(node_ast.offset);
            tuples.defpropsOffset(defprops_ast.offset);
            tuples.nsOffset(ns_ast.offset);
	...
	private final class AssignmentExpression_AST extends ScalarExpression_AST {
		...
		public int populateTuples(AwkTuples tuples) {
			...
			if (ast1 instanceof ID_AST) {
				...
				boolean isRef = false;
				if (id_ast.isArray()) {
				    if ((op == _EQUALS_) && (ast2 != null) && ((ast2 instanceof ArrayReference_AST) || 
				            ((ast2 instanceof ID_AST) && ast2.isArray()))) {
				        isRef = true;
				    } else {
	...
	// RMP
    private final class ArrayIndex_AST2 extends AST {

        private ArrayIndex_AST2(AST expr_ast, AST next) {
            super(expr_ast, next);
        }
...
//			}
	...
	private class AwkSymbolTableImpl {
		...
		AST addINTEGER(String integer) {
		    // RMP
			return new Integer_AST(Integer.decode(integer.trim()));
//            return new Integer_AST(Integer.parseInt(integer));
		}

		AST addDOUBLE(String dbl) {
		    // RMP
			return new Double_AST(Double.valueOf(dbl.trim()));
		}
