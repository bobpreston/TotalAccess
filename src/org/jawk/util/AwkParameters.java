...
//		String cls = mainClass.getName();
        String cls = com.jagacy.totalAccess.TotalAccess.class.getName();
        dest.println(com.jagacy.totalAccess.TotalAccess.VERSION);
        dest.println("Portions Copyright (c) Jagacy Software.");
		dest.println("usage:");
		dest.println(
				"java ... <-jar totalAccess.jar | " + cls + "> [-F fs_val]"
	...
	private static void addVariable(AwkSettings settings, String keyValue) {
		...
		try {
            ...
            value = Integer.decode(valueString.trim());
